...
def serialize_job(job, include_result_for_terminal: bool = False) -> dict:
    """Serialize a Job-like object into the JSON-friendly dict used by APIs and SSE.

    The function is defensive: it accepts either the in-memory `Job` dataclass,
    adapter record objects from persistence layers, or simple dict-like objects.

    By default, `result` is only included for terminal statuses (succeeded/failed/cancelled).
    Pass `include_result_for_terminal=True` to always include `result` when present.
    """
    try:
