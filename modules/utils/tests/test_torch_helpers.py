...
torch_is_mocked = isinstance(torch.rand, MagicMock) or hasattr(torch, '_mock_name')
...
class TestTorchHelpers(unittest.TestCase):
    ...
    def setUp(self):
        """Set up test fixtures and skip if torch is mocked."""
        if torch_is_mocked:
            self.skipTest("Torch is mocked - these tests require real torch functionality")
