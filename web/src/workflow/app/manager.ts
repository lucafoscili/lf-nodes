...
import { createComponent } from '../elements/components';
...
export class LfWorkflowApp {
  ...
  #STATE: WorkflowState = initState();
  #STRINGIFY = (value: Record<string, any>) => JSON.stringify(value, null, 2);
  //#endregion
  ...
  constructor() {
    ...
    this.#loadWorkflows()
      ...
      .catch((err) => {
        console.error(err);
        this.set.status('Unable to load workflows.', 'danger');
      });
  }
  //#endregion
  ...
  #initializeElements() {
    ...
    buttons.run.addEventListener('click', async () => {
      ...
      if (!workflowId) {
        this.set.status('Select a workflow to continue.', 'warning');
        return;
      }
      ...
      for (const [name, renderer] of this.#FIELD_RENDERERS.entries()) {
        ...
        if (val instanceof File) {
          ...
          try {
            ...
            this.set.status('File upload failed.', 'danger');
      ...
      try {
        ...
        if (!response.ok) {
          this.set.status(payload.detail ?? 'Workflow execution failed.', 'danger');
        ...
        const renderOutputForKey = (key: string | null) => {
          ...
          const str = this.#STRINGIFY(outputs[key] || outputs);
          const code = createComponent.code({
            lfLanguage: 'json',
            lfValue: str,
          });
          sections.result.appendChild(code);
        };
        ...
        console.error(error);
        this.set.status('Failed to execute workflow.', 'danger');
        if (sections.result) {
  ...
  set = {
    ...
    status: (message: string | null, tone: LfThemeUIState = 'info') => {
      const { ui } = this.#STATE;
      const { sections } = ui;
