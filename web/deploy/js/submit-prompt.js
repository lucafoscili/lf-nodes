...
if (!hasComfyApp) {
  let setStatus = function(message, tone = "info") {
  ...
  }, createFieldRenderer = function(field) {
  ...
  setStatus2 = setStatus, resetResult2 = resetResult, createFieldRenderer2 = createFieldRenderer;
  ...
  let workflows = [];
  const fieldRenderers = /* @__PURE__ */ new Map();
  ...
  if (runButton) {
    runButton.addEventListener("click", async () => {
      ...
      try {
        let renderOutputForKey2 = function(key) {
          if (!resultElement) return;
            ...
            if (val && val.images && Array.isArray(val.images) && val.images.length > 0) {
              ...
              if (filename) {
                const url = `/view?filename=${encodeURIComponent(
                  filename
                )}&type=${encodeURIComponent(type)}&subfolder=${encodeURIComponent(subfolder)}`;
        ...
        var renderOutputForKey = renderOutputForKey2;
  ...
  loadWorkflows().then(() => setStatus("Ready.", "info")).catch((err) => {
    console.error(err);
